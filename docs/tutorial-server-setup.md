...
5. dashboard는 최신 checkpoint의 best circle packing, score 변화, 로그를
   실시간에 가깝게 보여준다.
...
요청이 모두 느려질 수 있으므로 20B 모델에서는 3 또는 4부터 확인한다.
...
따라서 dashboard는 최신 checkpoint를 숫자 기준으로 찾아서 읽고, `best/`는 완료 후
최종 상태 확인용으로만 사용한다.

업데이트 루프:

1. `checkpoints/checkpoint_*` 중 숫자가 가장 큰 checkpoint를 찾는다.
2. `<checkpoint>/best_program_info.json`을 읽는다.
3. `iteration`은 top-level에서 읽고, score 값은 `metrics.combined_score`,
   `metrics.sum_radii`, `metrics.validity`에서 읽는다.
4. `<checkpoint>/best_program.py`를 별도 subprocess로 실행해서 `run_packing()` 결과를 얻는다.
5. `centers`, `radii`를 원 그림으로 렌더링한다.
6. checkpoint history로 best-so-far score plot을 그린다.
7. log tail은 별도로 파싱해서 invalid 후보, code extraction failure, timeout을 보여준다.
...
CHECKPOINT_DIR="$(find "$RUN_DIR/checkpoints" -maxdepth 1 -type d -name 'checkpoint_*' | sort -V | tail -1)"
...
이전 원격 서버 실험에서는 20 iterations가 약 24분 37초 걸렸으므로, 대략
1 iteration당 74초였다. 실제 대기 시간은 Ollama 동시 요청 수와 생성 길이에 따라
달라지므로 dashboard의 ETA는 처음에는 이 값을 쓰고, live job이 완료될수록
최근 완료 job의 평균으로 갱신한다.
