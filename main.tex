...
\title{Exploring Evolve-Style Coding Agents}
  ...
  \begin{center}
    ...
    \begin{tabular}{|p{3.6cm}|p{2.8cm}|p{3.4cm}|}
      ...
      Detects hidden patterns; suggests conjectures
        ...
        & LLM generates proofs and proof strategies \\
      \midrule
...
\begin{frame}
  ...
  Each carries a natural \emph{fitness} -- set size, tour length, sum of
  radii \(\Rightarrow\) a GA can search for the optimum.
  \pause
  \vspace{.25cm}
...
\begin{frame}
  ...
  \begin{itemize}
    \item[$\bullet$] \bemph{Cap set.} Evolve a function that decides which
      vectors to add next.
      \pause
    \item[$\bullet$] \bemph{Traveling salesman.} Evolve a heuristic that
      constructs short tours.
      \pause
    \item[$\bullet$] \bemph{Circle packing.} Evolve a constructor that places
