...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item[$\bullet$] \bemph{Ramsey} -- 2-coloring of \( K_N \) with no
      monochromatic clique.
    \item[$\bullet$] \bemph{Matrix multiplication} -- tensor decomposition
      of smallest rank.
  \end{itemize}
  \pause
  \vspace{.25cm}

  Each carries a natural \emph{fitness} -- set size, \# cliques, rank
  \(\Rightarrow\) a GA can search for the optimum.
  \pause
  \vspace{.25cm}
